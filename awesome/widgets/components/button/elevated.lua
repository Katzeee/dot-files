...
local wcontainer = require("widgets.components.container")
...
local function button(args)
	...
	args.shape = args.normal_shape or utils.ui.rounded_rect(beautiful.border_radius)
	...
	if args.hover_effect == true then
		utils.ui.add_hover_cursor(widget, "hand1")
	end
	...
	widget.animation = animation:new({
		...
		update = function(self, pos)
			if pos.color then
				widget:get_children_by_id("background_role")[1].bg = utils.color.rgb_to_hex(pos.color)
			end
			...
			if pos.border_color then
				widget:get_children_by_id("background_role")[1].border_color = utils.color.rgb_to_hex(
					pos.border_color
				)
			end
		end,
...
function elevated_button.state(args)
	...
	args.hover_bg = args.hover_bg or utils.color.button_color(args.normal_bg, 0.1)
	args.press_bg = args.press_bg or utils.color.button_color(args.normal_bg, 0.2)

	args.on_normal_bg = args.on_normal_bg or args.press_bg
	args.on_hover_bg = args.on_hover_bg or utils.color.button_color(args.on_normal_bg, 0.1)
	args.on_press_bg = args.on_press_bg or utils.color.button_color(args.on_normal_bg, 0.2)

	args.normal_shape = args.normal_shape or utils.ui.rounded_rect(beautiful.border_radius)
